...
std::priority_queue<std::pair<float, uint32_t>> flat_search(float* base, float* query, size_t base_number, size_t vecdim, size_t k) {
    ...
    constexpr size_t block_size = 16;  // 每次处理16个元素，分成6块（96/16=6）
    ...
    for (int j = 0; j < vec_blocks; ++j) {
        query_vec[j] = vld1q_f32(query + j * 4);  // 4个为一组存入寄存器
    }
    ...
    for (size_t i = 0; i < base_number; ++i) {
        ...
        for (size_t d = 0; d < vecdim; d += block_size) {
            ...
            // 使用之前加载的 query 向量进行计算
            sum1 = vmlaq_f32(sum1, b1, query_vec[d / 4]);
            sum2 = vmlaq_f32(sum2, b2, query_vec[(d + 4) / 4]);
            sum3 = vmlaq_f32(sum3, b3, query_vec[(d + 8) / 4]);
            sum4 = vmlaq_f32(sum4, b4, query_vec[(d + 12) / 4]);
        }
