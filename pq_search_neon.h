    ...
    uint32_t topp)
{
    ...
    for (uint32_t m = 0; m < SUBSPACE_NUM; ++m) {
        ...
        for (uint32_t k = 0; k < Ks; ++k) {
            ...
            float32_t c1 = *(code_sub + 4);
            float32_t q1 = *(query_sub + 4);
            float32_t sum1=vmlaq_f32(sum, q1, c1);
            /*
            q0 = vld1q_f32(query_sub + 4);
            c0 = vld1q_f32(code_sub + 4);
            sum = vmlaq_f32(sum, q0, c0);
            */
            float32_t c2 = *(code_sub + 8);
            float32_t q2 = *(query_sub + 8);
            float32_t sum2=vmlaq_f32(sum, q2, c2);
            /*
    ...
    for (size_t i = 0; i < candidate_count; ++i) {
        ...
        // 获取当前候选样本的起始地址
        const float* base_ptr = original_vectors + idx * 96;
